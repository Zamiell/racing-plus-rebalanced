export function main(projectile: EntityProjectile): void {
  if (!projectile.HasEntityFlags(EntityFlag.FLAG_FRIENDLY)) {
    return;
  }

  // Fade the projectiles of charmed enemies so that it is easier to see everything
  // We do this on every frame since the MC_POST_PROJECTILE_INIT callback is bugged
